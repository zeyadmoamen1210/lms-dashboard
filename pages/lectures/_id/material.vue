    ...
    <div class="container-fluid">
      ...
      <div class="d-flex flex-wrap path mb-3 mt-2">
        <template v-if="$auth.loggedIn">
          ...
          <div v-if="$route.query.level && $auth.user.role == 'admin'">
            <nuxt-link :to="`/levels/${$route.query.level}/subjects?level_name=${$route.query.level_name}`">
              {{ $route.query.level_name }}
            </nuxt-link>
          </div>
          <div class="divider" v-if="$route.query.level && $auth.user.role == 'admin'">
            <h6>
              <svg
          ...
          <div v-if="$route.query.subject_name && $route.query.subject">
            ...
            <nuxt-link :to="`/lectures?level=${$route.query.level}&level_name=${$route.query.level_name}&subject=${$route.query.subject}&subject_name=${$route.query.subject_name}`"> {{ $route.query.subject_name }} </nuxt-link>
          </div>
        ...
        <div v-if="$route.query.lecture_name">
          <h6>  {{$route.query.lecture_name }} </h6>
        </div>
      </div>
                ...
                $router.push(
                  `/bank-questions?${
                    ...
                    $route.query.subject
                      ...
                      : ''
                  }${$route.query.level_name ? '&level_name=' + $route.query.level_name : ''}
                  ${
                    $route.params.id ? '&lecture=' + $route.params.id : ''
                  }
                  ${$route.query.subject_name ? '&subject_name=' + $route.query.subject_name : ''}`
                )
              "
            >
...
export default {
  ...
  methods: {
    ...
    addVideo() {
      ...
      this.$axios
        .post(`/lecture/${this.$route.params.id}/material`, material)
        .then(res => {
          this.$message({
            message: this.$i18n.locale == 'ar' ? "تم إضافة الفديو بنجاح" : "Video Added Successfully",
            type: "success"
          });
    ...
    addFile() {
      ...
      this.$axios
        .post(`/lecture/${this.$route.params.id}/material`, formData)
        .then(res => {
          this.$message({
            message:
              this.$i18n.locale == "ar"
    ...
    openExamQuestion(e) {
      ...
      this.$router.push(`/exam-details/${e.id}?exam_name=${e.title}&subject=${this.$route.query.subject}&lecture=${this.$route.params.id}&subject_name=${this.$route.query.subject_name}&lecture_name=${this.$i18n.locale == 'ar' ? this.lesson.nameAr : this.lesson.nameEn}&level=${this.$route.query.level}&level_name=${this.$route.query.level_name}`)
    ...
    getSubjectExams() {
      const loading = this.$vs.loading();
      this.$axios
        .get(`/lecture/${this.$route.params.id}/exams?paginate=false`)
        .then(res => {
